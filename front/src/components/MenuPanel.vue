<template>
    <div class="menu fixed top-0 left-0 w-60 h-screen bg-gray-800 text-white">
        ...
        <button
            @click="logout"
            class="absolute bottom-4 left-4 w-10 h-10 text-white bg-red-500 rounded-full hover:bg-red-600 focus:outline-none"
        >
            <font-awesome-icon :icon="faSignOutAlt" />
        </button>
        <button
            @click="toggleMenu"
            class="absolute bottom-4 right-4 w-10 h-10 text-white bg-gray-600 rounded-full hover:bg-gray-700 focus:outline-none"
        >
